...
## 调用智能合约
> 智能合约的调用又分为两种，一种是正常交易，一种是合约调用。两者的区别是正常交易就是一笔普通交易，交易需要花费gas，矿工会将交易打包到区块上，对于合约状态的改变会持久化，例如你往合约写数据修改数据的场景，这种交易速度较慢，一般需要等待十几秒确认。另外一种就是合约调用，这种方式不会消耗gas，因为涉及写入区块以及改变合约状态，所以很快就返回结果，使用场景例如你读取合约的一些数据信息。
