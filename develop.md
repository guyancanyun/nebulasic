...
## 部署合约  
可以在命令行下进行部署，也可以使用web钱包部署更简单。  
###  第一步  
打开钱包导入keystore并输入密码解锁，将合约代码复制贴入编辑框。
![编译合约](resource/deploy_contact_1.png)
 
###  第二步  
点击测试，返回result和err内容全是空表示合约编译通过，然后再点击提交，随机可以看到生产了一笔交易的hash以及合约地址，记录好生成的合约地址，这是后面调用合约的唯一根据。
![提交交易](resource/deploy_contact_2.png)

###  第三步  
等待十几秒后查询部署合约的交易状态是否success。21个节点大概15秒出一个块，所以确认一笔交易大概十几秒。
![交易成功](resource/deploy_contact_3.png)

## 调用智能合约
> 智能合约的调用又分为两种，一种是正常交易，一种是合约调用。两者的区别是正常交易就是一笔普通交易，交易需要花费gas，矿工会将交易打包到区块上，对于合约状态的改变会持久化，例如你往合约写数据修改数据的场景，这种交易速度较慢，一般需要等待十几秒确认。另外一种就是合约调用，这种方式不会消耗gas，因为不涉及写入区块以及改变合约状态，所以很快就返回结果，使用场景例如你读取合约的一些数据信息。

**以Web钱包为例**  
调用合约的getBalance方法获得金额。需要填入方法名，以及参数（数组）。
![调用合约](resource/cal_contact_1.png)
点击测试，返回内容中result即我们想要查询的金额。  
如果需要发送正常交易，则还需要点击提交，然后等待交易完成。
![调用合约](resource/cal_contact_2.png)
